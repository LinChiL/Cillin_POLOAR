...
class POLOAR_ParticleUniverse:
    def __init__(self, total_points=1000, total_energy=20000.0, xi=1.0):
        ...
        # 奇点：能量集中在中心 20 个像素
        self.energies = torch.full((total_points,), 1e-9, device=self.device)
        mid = total_points // 2
        ...
        # 原子检测相关
        self.distance_history = defaultdict(list)  # 存储粒子对距离历史
    ...
    def is_stable_distance(self, pid1, pid2, current_dist):
        """检查粒子对的距离是否稳定"""
        ...
        min_dist = min(history)
        if min_dist == 0:
            return False
        
        change_percent = (max_dist - min_dist) / min_dist * 100
        return change_percent < 5.0  # 变化小于5%认为稳定
    ...
    def find_atoms(self):
        ...
        for pid, p in self.particles.items():
            ...
            for qid, q in self.particles.items():
                if pid == qid:
                    continue
                
                dist = abs(p.pos - q.pos)
                if dist < min_dist:
                    min_dist = dist
                    nearest = q
            
            if nearest and min_dist < 20:  # 距离小于20像素才考虑
                # 检查距离稳定性
                if self.is_stable_distance(pid, nearest.pid, min_dist):
                    # 避免重复记录
                    pair = tuple(sorted([pid, nearest.pid]))
                    if pair not in checked_pairs:
                        atoms.append((pid, nearest.pid, min_dist))
    ...
    def analyze_particle_type(self, grads, pos):
        ...
        profile = grads[left:right].cpu().numpy()
    ...
    def evolve(self, steps=50000, record_every=500):
        ...
        for step in range(steps):
            ...
            if step % record_every == 0:
                ...
                # 打印原子信息
                if atoms:
                    print(f"  Atoms detected:")
                    for pid1, pid2, dist in atoms[:3]:  # 只打印前3个
                        print(f"    Atom: particles {pid1}-{pid2}, distance={dist:.2f}")
    ...
    def visualize(self):
        ...
        # 1. 最终梯度分布
        ax = axes[0, 0]
        ax.plot(self.grads.cpu().numpy(), 'b-', linewidth=1)
        peaks, _ = self.find_particles(self.grads)
        ax.scatter(peaks, self.grads[peaks].cpu().numpy(), color='red', s=30, zorder=5)
        ...
        if self.history_grads:
            grad_history = torch.stack(self.history_grads).cpu().numpy()
        ...
        # 6. 能量分布
        ax = axes[1, 2]
        ax.plot(self.energies.cpu().numpy(), 'orange', linewidth=1)
...
particle_counts, particle_types = universe.evolve(steps=50000, record_every=500)
...
if final_atoms:
    ...
    for pid1, pid2, dist in final_atoms:
