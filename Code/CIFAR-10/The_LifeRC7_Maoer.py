...
class ResonantPredator(nn.Module):
    ...
    def forward_apex_search(self, x, steps=150, lr=0.2, temperature=1.2):
        device = x.device
        with torch.no_grad():
            feat_base = self.backbone(x).squeeze().detach()
        ...
        for i in range(steps):
            # 核心：引入随时间衰减的‘逻辑热流’
            current_temp = temperature * (1 - i/steps)
            noise = torch.randn_like(z) * current_temp
            
            logits = self.fc((z + noise).unsqueeze(0))
            
            # --- POLOAR 修正：平摊最强类别的引力 ---
            # 我们寻找前 5 个最可能的类别，强行给它们‘减压’
            top_values, top_indices = torch.topk(logits, 5)
            # 惩罚过强的尖峰，强制系统看‘非显著特征’
            penalty = top_values.mean()
            
            # 优化目标：寻找逻辑的一致性，而不是寻找最高分
            entropy = -torch.sum(F.softmax(logits, dim=1) * torch.log(F.softmax(logits, dim=1) + 1e-10))
            loss = -entropy + 0.1 * penalty
            ...
            with torch.no_grad():
                ...
                # 拓扑约束
                dist = torch.norm(z - feat_base)
                if dist > 8.0: z.copy_(feat_base + (z - feat_base) * (8.0 / dist))
...
def run_resonant_experiment():
    ...
    # 执行全局引力平摊搜索
    print("\n[Apex Search Suture] 开始全局引力平摊...")
    out_apex = predator.forward_apex_search(x, steps=150, lr=0.2, temperature=1.2)
    
    conf_apex, pred_apex = out_apex.max(1)
    
    # 诊断 Apex Search 的 L
    with torch.no_grad():
        ent_apex = -torch.sum(out_apex * torch.log(out_apex + 1e-10)).item()
        L_apex = (ent_apex / np.log(10)) / np.log10(1e5 / (ent_apex + 1e-7))

    print(f"【全局引力平摊】: 预测={pred_apex.item()}, 置信度={conf_apex.item():.4f}, 逻辑熵={ent_apex:.3f}, 激活L={L_apex:.3f}")
